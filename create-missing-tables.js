...
async function createMissingTables() {
  ...
  try {
    ...
    const sqlContent = fs.readFileSync('./server/migrations/create-enhanced-appointment-system.sql', 'utf8');
    ...
    try {
      const testQuery = `
        ...
        VALUES ('test-doctor-wallet', 400.00, 500.00, 300.00)
        ON CONFLICT(doctor_wallet) DO NOTHING
      `;
      
      await sequelize.query(testQuery);
      console.log('✅ Test pricing record created');
      
    } catch (error) {
      console.log('❌ Test pricing creation failed:', error.message);
    }
