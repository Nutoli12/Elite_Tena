    ...
    >
      ...
      >
        ...
        <div className="relative z-10">
          ...
          >
            {t('welcome')}, {user?.fullName || user?.profileData?.name || (user?.profileData?.firstName && user?.profileData?.lastName ? `${user.profileData.firstName} ${user.profileData.lastName}` : 'User')}!
          </motion.h1>
          <motion.p
            initial={{ x: -20, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
      ...
      {/* Quick Actions */}
      <QuickActions
        actions={navigationService.getPatientQuickActions()}
        delay={0.9}
      />
    </motion.div>
  );
