...
const fixUnknownDoctorDirectly = async () => {
  ...
  try {
    ...
    const orphanedQuery = `
      SELECT a.id, a."doctorWallet", a."patientWallet", a."appointmentDate"
      FROM appointments a
      LEFT JOIN doctors d ON LOWER(a."doctorWallet") = LOWER(d."walletAddress")
    ...
    if (orphanedAppointments.length > 0) {
      ...
      orphanedAppointments.slice(0, 3).forEach((apt, index) => {
        ...
        console.log(`      Doctor Wallet: ${apt.doctorWallet}`);
        console.log(`      Patient Wallet: ${apt.patientWallet}`);
    ...
    const uniqueDoctorsQuery = `
      SELECT DISTINCT a."doctorWallet"
      FROM appointments a
      LEFT JOIN doctors d ON LOWER(a."doctorWallet") = LOWER(d."walletAddress")
      WHERE d."walletAddress" IS NULL
      AND a."doctorWallet" IS NOT NULL
      AND a."doctorWallet" != '';
    `;
    ...
    const statsQuery = `
      SELECT 
        ...
        (SELECT COUNT(*) FROM appointments a 
         INNER JOIN doctors d ON LOWER(a."doctorWallet") = LOWER(d."walletAddress")) as appointments_with_doctors;
    `;
