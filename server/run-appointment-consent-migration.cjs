...
async function runMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, 'migrations/add-appointment-specific-consent-workflow.sql');
    ...
    // Use the simple migration instead
    const simpleMigrationPath = path.join(__dirname, 'migrations/add-appointment-consent-simple.sql');
    ...
    const dbConfig = {
      ...
      database: process.env.DB_NAME || 'elitetena',
    ...
    console.log('   2. Test the appointment consent workflow');
